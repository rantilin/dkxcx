	...
	<view>
		...
		<view class="taskpostcont">
			...
			<view class="listupload" style="display:flex;flex-wrap: wrap;">
				<u-image v-for="(item,index) in imgList" :key='index' style="margin-left:5rpx" :src="item" width="206"
					height="186" class="item"></u-image>
			</view>
			<view class="listbut">
			...
			post() {
				...
				if (that.status == 0) {
					...
					this.$api.parentPelease(data, (res) => {
						if (res.code == 200) {
							uni.navigateTo({
								url:'../success/sign/sign?info='+JSON.stringify(res.datas.integral)
							})
						} else {
							uni.showToast({
			...
			getDraft() {
				...
				this.$api.parentPelease(data, (res) => {
					if (res.datas.error == "参数错误") {
						...
						uni.showModal({
							title: "提示",
							content: "您上次编辑的打开任务尚未完成，您是继续编辑上次未完成的内容",
							confirmText: "继续编辑",
							cancelText: "放弃草稿",
							confirmColor: "#333333",
							cancelColor: "#666666",
